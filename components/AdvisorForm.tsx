...
import type { Advisor, AdvisorInsert } from '@/types'

type Props = { advisor?: Advisor }

export default function AdvisorForm({ advisor }: Props) {
  const router = useRouter()
  const [loading, setLoading] = useState(false)
  const [form, setForm] = useState<Partial<AdvisorInsert>>({
    name: advisor?.name ?? '',
    name_kana: advisor?.name_kana ?? '',
    office_name: advisor?.office_name ?? '',
    prefecture: '兵庫県',
    city: advisor?.city ?? '',
    phone: advisor?.phone ?? '',
    email: advisor?.email ?? '',
    ...
    is_published: advisor?.is_published ?? true,
  })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const url = advisor ? `/api/advisors/${advisor.id}` : '/api/advisors'
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-sm border transition-colors ${
                form.specialties?.includes(s)
                  ? 'bg-hyogo-700 text-white border-hyogo-700'
                  : 'border-stone-300 text-gray-600 hover:border-hyogo-400'
              }`}
            >
              {s}
            </button>
      ...
      <div className="flex justify-end gap-3">
        <button type="button" onClick={() => router.back()} className="btn-outline">キャンセル</button>
        <button type="submit" disabled={loading} className="btn-primary">
          {loading ? '保存中...' : advisor ? '変更を保存' : '新規登録'}
        </button>
      </div>
    </form>
  )
